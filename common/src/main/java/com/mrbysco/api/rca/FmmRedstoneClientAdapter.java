...
public class FmmRedstoneClientAdapter {
	...
	private static class FileMemMap {
		...
		public void remove() {
			try {
				@SuppressWarnings("unused") boolean b = new File(mapFilePath).delete();
			} catch (Throwable ignored) {
			}
		}
	...
	public static class Adapter implements RedstoneClientAdapter {
		...
		public static String ipcIoPath(boolean mcSideOutput) {
			return Auxiliaries.getGameDirectory().resolve("redstonepen." + (mcSideOutput ? 'o' : 'i') + ".mmap").toString();
		} // Before 1.21: Path.of(System.getProperty("java.io.tmpdir")
