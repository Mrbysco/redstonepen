...
public class GuiTextEditing {
	...
	public static class MultiLineTextBox extends Guis.UiWidget implements GuiEventListener {
		...
		public MultiLineTextBox setCursorColor(int color) {
			cursor_color_ = color | 0xff000000;
			return this;
		}
		...
		public boolean mouseClicked(double x, double y, int button) {
			if ((!active) || (!visible) || (x < getX()) || (y < getY()) || (x > getX() + this.width) || (y > getY() + this.height))
				return false;
		...
		private boolean specialKeyMatched(int key) {
			...
			switch (key) {
				...
				case 268 -> {
					edit_.setCursorPos(getDisplayCache().findLineStart(edit_.getCursorPos()), Screen.hasShiftDown());
					return true;
				} // home hey
				case 269 -> {
					edit_.setCursorPos(getDisplayCache().findLineEnd(edit_.getCursorPos()), Screen.hasShiftDown());
					return true;
				} // end key
				default -> {
					return false;
		...
		private DisplayCache getDisplayCache() {
			if (display_cache_ == null) {
				display_cache_ = rebuildDisplayCache();
			}
		...
		private static int findLineFromPos(int[] line_starts, int cursor_pos) {
			final int i = Arrays.binarySearch(line_starts, cursor_pos);
			return (i < 0) ? (-(i + 2)) : (i);
		}
		...
		static class LineInfo {
			...
			public LineInfo(Style fs, String s, int x0, int y0) {
				...
				x = x0;
				y = y0;
				asComponent = (Component.literal(contents)).setStyle(style);
			}
		}
